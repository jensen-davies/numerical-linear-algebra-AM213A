...
subroutine SymmToTri(A)
  implicit none

  real,allocatable,dimension(:,:), intent(INOUT) :: A
  real,allocatable,dimension(:,:) :: outerprod
  real,allocatable, dimension(:) :: v
  integer :: i,j,k,msize
  real :: s, A_norm, v_norm

  !This algorithm takes in a symmetric matrix A and outputs a tridiagonal matrix. The code follows
  !the pseudo code provided in Chapter 4 for Hessenberg form. In the case A isn't symmetric, the algorithm
  !outputs an upper-Hessenberg matrix.
  ...
  do j = 1,msize-1
    ...
    do i = j+1,msize
      A_norm = A_norm + A(i,j)**2
    end do
    A_norm = sqrt(A_norm)

    s = sign(A_norm, A(j+1,j))
    ...
    do i = 1,msize
      do k = 1,msize
      outerprod(k,i) = v(k)*v(i)
      end do
    end do
    
    A = A - 2.0*matmul(outerprod,A)
    A = A - 2.0*matmul(A,outerprod)
...
end subroutine SymmToTri

subroutine QR_eigenvalues(A)
  implicit none
  ...
  !This algorithm calculates the eigenvalues of input matrix A via the QR algorithm. The algorithm gradually
  !transforms A into a diagonal matrix via Householder reflections, with diagonal elements being the eigenvalues
  !of matrix A.

  msize = size(A,1)

  !Calculates the Frobenius norm of input A, excluding the diagonal elements.
  error = 0.0
  do i = 1,msize
    do j = 1,msize
    error = error+ A(i,j)**2
...
subroutine QR_eigenvalues_shift_deflation(A)

  implicit none

  real,allocatable,dimension(:,:), intent(INOUT):: A
  real,allocatable,dimension(:,:) :: Q,R,Id,B
  real ::error,mu
  logical :: singular
  integer :: msize,i,j
  
  !This algorithm calculates and outputs the eigenvalues of input matrix A via the QR algorithm, using shifts
  !and deflation. The algorithm gradually transforms A into a diagonal matrix via Householder transformations,
  !where the diagonal entries of said matrix are the eigenvalues of A. The convergence of this method is much
  !faster than performing the QR algorithm without shifts and deflation.
  !This code nearly exactly follows the pseudocode provided in Chapter 4.
...
end subroutine QR_eigenvalues_shift_deflation

subroutine Inverse_Iteration(A,mu, x)
  implicit none

  real, allocatable, dimension(:,:), intent(IN) :: A
  real, allocatable, dimension(:,:) :: B,Id,y,r,s,x_sol
  real, intent(IN) :: mu
  real, allocatable, dimension(:,:), intent(OUT) :: x
  real,allocatable,dimension(:) :: temp
  real :: r_norm, Bx_norm,y_norm,s_norm
  integer :: msize, i,j
  logical :: singular

  !This subroutine calculates the eigenvectors of input matrix A for a given input eigenvalue approximation mu.
  !The output is the eigenvector x, which is associated with eigenvalue mu.
  !The algorithm nearly exactly follows the provided pseudocode in Chapter 4.

  msize = size(A,1)
  allocate(B(msize,msize))
  allocate(Id(msize,msize))
  allocate(x(msize,1))
  allocate(x_sol(msize,1))
  allocate(r(msize,1))
  allocate(s(msize,1))
  allocate(y(msize,1))
  allocate(temp(msize))
  ...
  !Perform Inverse Iteration Algorithm from Ch. 4 notes.
  do while (r_norm > 1.0E-4 .AND. s_norm > 1.0E-4)
    !Solve By = x, where B = (A-mu*Id).
    B = A - mu*Id
    x_sol = x
    call GEPP(B,x_sol,msize,1,singular)
    call backsub(B,x_sol,y,msize,1)
    temp = y(:,1)
    call stdNorm(temp,size(y), y_norm)
    ...
    !Update the norm of r and s
    temp = r(:,1)
    call stdNorm(temp,size(r), r_norm)

    temp = s(:,1)
    call stdNorm(temp, size(s), s_norm)
